...
weather lag 2 - but less weight for lag 1 and 2 mimicing g2
SST
'''
...
class genesis_pred(object):
    ...
    def fit(self, atl):
        ...
        for combi in itertools.product(*[coordinates[var] for var in variables]):
            ...
            distance.loc[:,'weather_1'] = weather_dis[int(point[np.array(variables)=='weather_1'])].values ** 0.5
            distance.loc[:,'weather_2'] = weather_dis[int(point[np.array(variables)=='weather_2'])].values ** 0.5
