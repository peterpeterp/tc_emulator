...
class wind_estimator(object):
    ...
    def get_analogue_pdfs(self, atl):
        coordinates = {
            'sst_rel':np.arange(1,3.5,0.4).round(2),
        ...
        variables = [v for v in list(coordinates.keys()) if v not in ['sst_rel','wind']]
        pdfs = xr.DataArray( coords=coordinates, dims=variables+['sst_rel','wind'])
        ...
        quantiles, wind_quR_params = sst_vs_wind_quantile_regression(self._tracks, plot_dir=self._dir, sst_var='sst_rel')

        for sst_mod in coordinates['sst_rel']:
            # get modified tracks with transformed wind and wind_before according to QR
            modified_tracks = mod_tracks_sst(self._tracks, sst_mod, wind_quR_params, quantiles, sst_var='sst_rel')
